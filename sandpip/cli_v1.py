...
def resolve_allowed_ips(domains: list[str]) -> list[str]:
    ...
    for domain in domains:
        try:
            ...
            print(f"sandpip_v1: warning: could not resolve {domain}: {exc}", file=sys.stderr)
            continue
...
def main() -> int:
    ...
    if not library.exists():
        print(
            f"sandpip_v1: missing {library}; run `make` first",
            file=sys.stderr,
        )
        return 2
    ...
    print("sandpip: forcing v1 user-space isolation engine (LD_PRELOAD)", file=sys.stderr)
